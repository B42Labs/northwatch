<script lang="ts">
  ...
  function totalChanges(diff: DiffResult): number {
    return diff.tables.reduce(
      (sum, t) =>
        sum +
        (t.added?.length ?? 0) +
        (t.removed?.length ?? 0) +
...
<div class="rounded-lg border border-base-300 bg-base-100 p-4">
  <div class="mb-3 flex items-center justify-between">
    ...
    <span class="badge badge-ghost">{totalChanges(diff)} changes</span>
  </div>
  ...
  {:else}
    <div class="flex flex-col gap-4">
      {#each diff.tables as tableDiff}
        <div class="rounded border border-base-300 p-3">
          <div class="mb-2 flex items-center gap-2 font-medium">
            <span class="badge badge-ghost badge-sm">{tableDiff.database}</span>
            <span>{tableDiff.table}</span>
            <span class="ml-auto flex gap-2 text-xs">
              {#if tableDiff.added?.length}
                <span class="text-success">+{tableDiff.added.length} added</span
                >
              {/if}
          ...
          {#if tableDiff.added?.length}
            <div class="mb-2">
              ...
              {#each tableDiff.added as row}
                <pre
                  class="mb-1 max-h-32 overflow-auto rounded border-l-2 border-success bg-success/5 p-2 text-xs">{JSON.stringify(
                    row,
                    null,
                    2,
                  )}</pre>
          ...
          {#if tableDiff.removed?.length}
            <div class="mb-2">
              ...
              {#each tableDiff.removed as row}
                <pre
                  class="mb-1 max-h-32 overflow-auto rounded border-l-2 border-error bg-error/5 p-2 text-xs">{JSON.stringify(
                    row,
                    null,
                    2,
                  )}</pre>
          ...
          {#if tableDiff.modified?.length}
            <div>
              ...
              {#each tableDiff.modified as mod}
                <div
