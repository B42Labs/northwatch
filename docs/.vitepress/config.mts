...
export default defineConfig({
  ...
  themeConfig: {
    ...
    // One sidebar for the whole site rather than a per-section one, so the
    // Overview at `/` — the page readers land on — has navigation, and every
    // page can reach every other section without going through the nav bar.
    // The how-to guides are grouped and collapsed to keep the list scannable;
    // VitePress expands whichever group holds the active page.
    sidebar: [
      { text: 'Overview', link: '/' },
      {
        text: 'Tutorials',
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            items: [
              {
                text: 'Run the local lab',
                link: '/how-to/run-the-local-lab',
              },
              {
