...
- The chassis staleness threshold is global, set with
  `--chassis-stale-threshold`, not in the file. Beyond the chassis inventory, it
  also drives gateway HA-member liveness (a stale member is not a viable
  election candidate) and the `stale_chassis_config` alert. See
  [Chassis inventory](/reference/cli#chassis-inventory) for the semantics.
