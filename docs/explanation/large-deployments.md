...
The important limitation: staging spreads the *many* tables across time, but the
*one dominant table* is still delivered as a single reply. Batching does not chunk
*within* a table — so on a database dominated by `Logical_Flow`, staging helps the
long tail of tables but not the single biggest transfer.
...
a single request. Only live connections — server startup and snapshot capture —
are tuned.
