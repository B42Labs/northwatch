...
holding both in your head at once. Northwatch makes that join a first-class
feature: correlation links the two databases, and Omnisearch is the entry point
that uses those links.
...
do not: intent with no realization (a port that never bound), or realization
