...
disables) and deletes the rest. Snapshots you took deliberately, whether manual,
labeled or imported, are exempt, because retention must never reclaim something an
...
Two different things both go by the name "snapshot":
...
every read/debug subsystem against that copy, with no live OVN connection at all.
See [Explore a deployment offline](/tutorials/explore-a-deployment-offline).
