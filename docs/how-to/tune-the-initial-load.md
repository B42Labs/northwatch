...
table in one atomic message, because OVSDB has no pagination. On a large Southbound
...
Two flags bound this load. Both apply to live server mode and to snapshot
...
`delay × table count`), so a large `--monitor-batch-delay` (for example `1s` on a
huge database) no longer risks a fixed timeout aborting startup mid-load.
