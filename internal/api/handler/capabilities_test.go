...
func TestCapabilities(t *testing.T) {
	...
	RegisterCapabilities(mux, false, false, false, false, nil)
	...
	assert.NotContains(t, caps, "ovs")
	assert.NotContains(t, caps, "snapshot")
	assert.Equal(t, "live", body["mode"])
	assert.NotContains(t, body, "snapshot")
}

func TestCapabilities_WithOVS(t *testing.T) {
	mux := http.NewServeMux()
	RegisterCapabilities(mux, false, false, false, true, nil)
	...
	caps, ok := body["capabilities"].([]any)
	require.True(t, ok)
	assert.Contains(t, caps, "ovs")
}

func TestCapabilities_WithSnapshot(t *testing.T) {
	mux := http.NewServeMux()
	RegisterCapabilities(mux, false, false, false, false, &SnapshotInfo{
...
func TestCapabilities_WithEnrichment(t *testing.T) {
	mux := http.NewServeMux()
	RegisterCapabilities(mux, true, false, false, false, nil)
...
func TestCapabilities_WithMultiCluster(t *testing.T) {
	mux := http.NewServeMux()
	RegisterCapabilities(mux, false, false, true, false, nil)
